...
class CSP:
	...
	self.numOfFailures = 0
	self.numOfBacktracks = 0
	...
	for value in assignment[var]:
		...
		if value in self.domains[var]:
			inference = self.inference(temp_assignment, self.get_all_arcs())			
			if inference:
				solution = self.backtrack(temp_assignment)
				
				if solution is not False:
					return solution
	self.numOfFailures += 1
	...
	while queue:
		...
		if self.revise(assignment, x, y):
			...
			for neighbor in self.get_all_neighboring_arcs(x):
				if cmp(neighbor, (x,y)) is not 0:				
...
def run():
	...
	print '\nNum of failures: ' + str(csp.numOfFailures)
	print '\nNum of backtracks: ' + str(csp.numOfBacktracks)
run()
